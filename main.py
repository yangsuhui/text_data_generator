...
flags = parse_args()   ##程序運行時超參數，例如指定配置文件路徑、語料等
cfg = load_config(flags.config_file)   ##cfg是字典，數據生成時的配置，比如是否加noise，line等

##fonts是需要使用的font字體文件的路徑組成的list
fonts = font_utils.get_font_paths_from_list(flags.fonts_list)

##bgs是list，每個元素是cv讀取的bg圖片
...
# for num_i in range(len(bgs)):
#     fname_bg_ori = os.path.join(flags.save_dir, 'bg_{}'.format(num_i) + '.png')
#     cv2.imwrite(fname_bg_ori, bgs[num_i])
...
def generate_img(img_index, q=None):
    ...
    if not flags.viz:
        ...
        #fname_bg_ori_word_img = os.path.join(flags.save_dir, base_name + '_bg_ori_word_img.png')
        ...
        f_det = open(fname_det, mode='a', encoding='utf-8')
        coords = "{},{}".format(','.join([str(j) for i in text_box_pnts_ori for j in i]), word)
        f_det.write(coords + '\n')
