...
class Renderer(object):
    def __init__(self, corpus, fonts, bgs, cfg, width=256, height=32,
        ...
        self.max_chars = math.floor(width / 4) - 1  ##CRNN下採樣4倍，容許的最大char個數
        self.debug = debug
        self.gpu = gpu
        self.strict = strict
        self.cfg = cfg
        ...
        #self.is_bgr()返回cfg配置文件中font或者line的color是否true，如果為false，則將bg背景圖片轉成灰度圖
        if not self.is_bgr():
            for i, bg in enumerate(self.bgs):
                self.bgs[i] = cv2.cvtColor(bg, cv2.COLOR_BGR2GRAY)

        ##self.font_unsupport_chars是個字典，key是font的路徑，value是對應的font中不支持的char_file中的字符
        if self.strict:
    ...
    def gen_img(self, img_index, direction='horizonal'):
        ...
        if self.debug:
            bg_ori_word_img = draw_box(bg_ori, text_box_pnts_ori, (155, 255, 0))
        ...
        #return word_img, word, bg, bg_ori_word_img, text_box_pnts_ori
        return word_img, word, bg, bg_ori, text_box_pnts_ori
    ...
    def gen_bg(self, width, height):
        ...
        if apply(self.cfg.img_bg):
            ##從背景圖片中產生背景，尺寸為[int(width), int(height)]
            print('+++++++++')
            bg, coordinates_one, bg_ori = self.gen_bg_from_image(int(width), int(height))
        else:
            bg = self.gen_rand_bg(int(width), int(height))
        return bg, coordinates_one, bg_ori
    ...
    def gen_bg_from_image(self, width, height):
        ...
        # path_i = '/data/nfs/yangsuhui/data/complex_font_generater_four/train/images'
        # fname_bg = os.path.join(path_i, str(num_c_i) + '_bg.png')
        # cv2.imwrite(fname_bg, bg)
        ...
        # fname_bg_out = os.path.join(path_i, str(num_c_i) + '_bg_out.png')
        # cv2.imwrite(fname_bg_out, out)
        out_ori = copy.deepcopy(out)

        out = out[y_offset:y_offset + height, x_offset:x_offset + width]

        coordinates_one = [y_offset, y_offset + height, x_offset, x_offset + width]

        # fname_bg_out = os.path.join(path_i, str(num_c_i) + '_bg_out_cut.png')
        # cv2.imwrite(fname_bg_out, out)
        ...
        return out, coordinates_one, out_ori
